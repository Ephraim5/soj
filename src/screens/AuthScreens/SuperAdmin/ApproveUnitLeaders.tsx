...
import { View, Text, StyleSheet, TouchableOpacity, SafeAreaView, ScrollView, Image, ActivityIndicator, Modal, StatusBar } from 'react-native';
...
interface PendingUser { _id: string; firstName: string; surname: string; middleName?: string; email?: string; phone?: string; unitLeaderUnit?: { _id: string; name: string } | null; roles: { role: string; unit?: { _id: string; name: string } | null }[] }

const ApproveUnitLeadersScreen: React.FC = () => {
    ...
    const fetchPending = async () => {
        try {
            ...
            if (!token) return;
            const resp = await axios.get(`${BASE_URl}/api/users/pending/list?type=unit-leaders`, { headers: { Authorization: `Bearer ${token}` } });
            const list: PendingUser[] = resp.data.users || [];
            // Only keep those that actually have UnitLeader role
            const filtered = list.filter(u => (u.roles || []).some(r => r.role === 'UnitLeader'));
            setUsers(filtered);
        } catch (e) {
            console.log('fetch pending leaders error', e);
    ...
    const approve = async () => {
        ...
        try {
            ...
            closeModal();
        } catch (e) { console.log('approve error', e); setSubmitting(false); }
    };
    ...
    const deny = async () => {
        ...
        try {
            ...
            if (resp.data.ok) { setUsers(u => u.filter(x => x._id !== target._id)); }
            closeModal();
        } catch (e) { console.log('deny error', e); setSubmitting(false); }
    };
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={[styles.header, { paddingTop: Math.max(insets.top, 12) }]}> 
                ...
                <Text style={styles.headerTitle}>Approve Unit Leaders</Text>
            </View>
            <ScrollView
              contentContainerStyle={[styles.container, empty && styles.emptyContent]}
            ...
            >
                ...
                {empty && (
                    <View style={styles.emptyBox}>
                        ...
                        <Text style={styles.emptyText}>No pending unit leader registrations</Text>
                    </View>
                )}
                {users.map(u => {
                    const unitName = u.unitLeaderUnit?.name || (u.roles.find(r => r.role === 'UnitLeader')?.unit?.name) || '';
                    return (
                        <View key={u._id} style={styles.card}>
                            ...
                            <View style={{ flex: 1 }}>
                                <Text style={styles.name}>{u.firstName} {u.surname} | <Text style={styles.unit}>{unitName}</Text></Text>
                ...
                <View style={styles.modalBackdrop}>
                    <View style={styles.modalCard}>
                        ...
                        <Text style={styles.modalQuestion}>
                            {modalType === 'approve' ? 'Are you sure you want to approve this registration?' : 'Are you sure you want to deny this registration?'}
                        </Text>
                        <Text style={styles.modalBody}>
                            {modalType === 'approve' ? 'This action will confirm ' : 'If denied, '}
                            <Text style={styles.bold}>{target?.firstName} {target?.surname}</Text>
                            {modalType === 'approve' ? ' as the unit leader for ' : `’s registration as Unit leader for `}
                            <Text style={styles.bold}>{target?.unitLeaderUnit?.name || 'this unit'}</Text>
                            {modalType === 'approve' ? '.' : ' will be permanently deleted from the system and cannot be restored.'}
                        </Text>
...
const styles = StyleSheet.create({
    ...
    topLabel: { fontSize: 16, fontWeight: '600', color: '#111827', marginBottom: 16 },
...
export default ApproveUnitLeadersScreen;
