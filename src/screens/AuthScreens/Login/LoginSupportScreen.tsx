...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'clearError':
      ...
      const { [action.field]: _omit, ...rest } = state.errors; return { ...state, errors: rest };
    case 'bulkErrors':
...
export default function LoginSupportScreen() {
  ...
  <StatusBar barStyle="dark-content" />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingBottom: heightPercentageToDP(10), paddingTop: heightPercentageToDP(10) }} keyboardShouldPersistTaps="handled">
        ...
        <Text style={{ color: '#4a4a4a', fontSize: 14, lineHeight: 20, marginBottom: 22 }}>
          This support page helps you quickly resolve any technical issue or challenge you're experiencing. Technical support for this app is provided directly by <Text style={{ fontWeight: '700' }}>Skyrazor Digital Limited</Text>. Please submit your issue below, and we'll get back to you promptly.
        </Text>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
          {roles.map(r => (
            <TouchableOpacity key={r} onPress={() => setRole(r)} style={[styles.rolePill, role === r && { backgroundColor: Colors.primary }]}>
              <Text style={[styles.rolePillText, role === r && { color: '#fff' }]}>{r}</Text>
            </TouchableOpacity>
          ))}
        </View>
...
const styles = {
  ...
  rolePill: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 16, backgroundColor: '#eef6f9' },
  rolePillText: { color: Colors.primary, fontWeight: '600', fontSize: 12 },
} as const;
